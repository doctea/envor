...
*/
  for (int i = 0 ; i < NUM_ENVELOPES ; i++) {
    envelopes[i].attack_length = 0;
    envelopes[i].decay_length = 64;
  }
  //envelopes[1].invert = true;
}
...
void process_envelopes(unsigned long now) {
  //unsigned long envelope_time = bpm_clock(); //millis();
  ...
  //
    ...
    if (s==ATTACK) {
        ...
        if (envelopes[i].release_length>0) {
          ...
          if (i==0) {
            Serial.print("in RELEASE stag, elapsedRelease is ");
            Serial.print(eR);
            Serial.println(" - trigger envelope 1 as reached release stage of envelope 0");
            if (eR==0.0)
        ...
        if (elapsed > envelopes[i].release_length || envelopes[i].release_length==0) {
          ...
          Serial.print("--> Leaving RELEASE stage at elapsed ");
    ...
    unsigned long level_to_send = envelopes[i].invert ? 127-lvl : lvl;

    if (envelopes[i].last_sent_actual_lvl != level_to_send) { //envelopes[i].invert ? 127-lvl : lvl) {  // only send a new value if its different to the last value sent for this envelope
      ...
      /*Serial.print("Level to send for ");
      Serial.print(i);
      Serial.print(" is ");
      Serial.println(level_to_send);*/

      if (envelopes[i].invert && s==OFF) {
        level_to_send = 0;
      }
      
      if (i==0) {
        //int to_send = envelopes[i].invert ? 127-lvl : lvl;
        MCP.fastWriteA(map(level_to_send, 0, 127, 0, MCP.maxValue()*0.8)); //2048)); //MCP.maxValue()));
      } else {
        //int to_send = envelopes[i].invert ? 127-lvl : lvl;
        MCP.fastWriteB(map(level_to_send, 0, 127, 0, MCP.maxValue()*0.8)); //2048)); //MCP.maxValue()));
      }
