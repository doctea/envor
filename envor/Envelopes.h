...
#define NUM_ENVELOPES_EXTENDED 2
#endif
...
/*#else
  ...
  unsigned int  hold_length     = 0; //(PPQN / 4) - 1;   // H - hold    - length to hold at end of attack before decay
  unsigned int  decay_length    = 0; //(PPQN / 2) - 1;   // D - decay   - length of stage
